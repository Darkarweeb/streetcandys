/**
 * Street Candy — API de Estimación de Impuesto
 * GET /api/carrito/impuesto?pais=CO&session_id=xxx
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    const resultado = await carritoServicio.estimarImpuesto(carritoId, codigoPais);
    if (!resultado.exito) {
      return NextResponse.json(resultado, { status: 400 });
    }
    return NextResponse.json(resultado);
  } catch (error) {
    console.error('[GET /api/carrito/impuesto]', error);
    return NextResponse.json(
