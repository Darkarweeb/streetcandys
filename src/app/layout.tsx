  ...
  return (
    <html lang="es" className={dmSans.variable}>
      <head>
        ...
        <script
          dangerouslySetInnerHTML={{
            __html: `(function(){var RCK='__sc_chunk_reload_count__';var MAX=3;function rc(){try{return parseInt(sessionStorage.getItem(RCK)||'0',10);}catch(e){return 0;}}function ic(){try{var c=rc()+1;sessionStorage.setItem(RCK,String(c));}catch(e){}}function clr(){try{if(window.caches){window.caches.keys().then(function(k){k.forEach(function(n){window.caches.delete(n);});});}}catch(e){}try{var ls=window.localStorage;if(ls){var tr=[];for(var i=0;i<ls.length;i++){var k=ls.key(i);if(k&&(k.indexOf('__RSC_')!==-1||k.indexOf('next-router')!==-1||k.indexOf('_next')!==-1||k.indexOf('__NEXT_')!==-1)){tr.push(k);}}tr.forEach(function(k){ls.removeItem(k);});}}catch(e){}try{var ss=window.sessionStorage;if(ss){var ts=[];for(var j=0;j<ss.length;j++){var sk=ss.key(j);if(sk&&sk!==RCK&&(sk.indexOf('__RSC_')!==-1||sk.indexOf('next-router')!==-1||sk.indexOf('_next')!==-1||sk.indexOf('__NEXT_')!==-1)){ts.push(sk);}}ts.forEach(function(k){ss.removeItem(k);});}}catch(e){}}function isChunk(m){return m.indexOf('originalFactory.call')!==-1||m.indexOf("reading 'call'")!==-1||m.indexOf('Loading chunk')!==-1||m.indexOf('ChunkLoadError')!==-1||m.indexOf('originalFactory')!==-1;}function doReload(){if(rc()>=MAX){return;}ic();clr();window.location.reload();}var _o=window.onerror;window.onerror=function(m,s,l,c,e){if(isChunk(String(m||''))){doReload();return true;}if(_o)return _o.apply(this,arguments);};window.addEventListener('unhandledrejection',function(ev){var ms=String(ev&&ev.reason&&ev.reason.message||ev&&ev.reason||'');if(isChunk(ms)){ev.preventDefault();doReload();}});setTimeout(function(){try{sessionStorage.removeItem(RCK);}catch(e){}},8000);})();`,
          }}
        />
